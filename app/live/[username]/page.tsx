...
import { User, MessageSquare, Share2, Users, Clock, Eye } from 'lucide-react';
import { toast } from 'sonner';
import { formatDistanceToNow } from 'date-fns';
import { useAuth } from '@/context/AuthContext';
import SubscribeButton from '@/components/SubscribeButton';
import { POLLING_INTERVALS } from '@/lib/constants';
import { getHlsUrl, extractApiData } from '@/lib/stream-utils';
import type { StreamData, LiveStream } from '@/types';
...
export default function LivePage({ params }: LivePageProps) {
    ...
    useEffect(() => {
        if (streamData?.stream && streamData.stream.status === 'live') {
            ...
            }, POLLING_INTERVALS.STREAM_STATUS);
    ...
    const fetchStreamData = async (silent = false) => {
        try {
            ...
            const channelData = extractApiData<any>(res);
            ...
            if (channelData.user?.isLive) {
                ...
                const liveStreams = extractApiData<LiveStream[]>(streamsRes) || [];
                streamInfo = liveStreams.find((s) => s.user.username === username);
            }
    ...
    const hlsUrl = getHlsUrl(streamKey, isLive);
                        ...
                        <div className="flex justify-between items-start gap-4">
                            ...
                            <div className="flex gap-2 flex-shrink-0">
                                {channel && currentUser && channel.id !== currentUser.id && (
                                    <SubscribeButton channelName={channel.name} channelId={channel.id} />
                                )}
